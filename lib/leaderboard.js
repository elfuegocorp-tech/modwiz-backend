...
async function computeWeeklyXpRanking(weekStartUtc, weekEndUtc) {
  ...
  for (const e of events) {
    totals.set(e.wp_user_id, (totals.get(e.wp_user_id) ?? 0) + e.xp_awarded);
  }
  ...
  const { data: states, error: statesError } = await supabase
    ...
    .select('wp_user_id, streak_count, souls_balance, first_name')
    .in('wp_user_id', wpUserIds);
  if (statesError) throw statesError;
  const stateById = new Map(states.map((s) => [s.wp_user_id, s]));
...
async function maybeGrantWeeklyRewards() {
  ...
  const ranking = await computeWeeklyXpRanking(prevWeekStart, prevWeekEnd);
  for (let i = 0; i < Math.min(3, ranking.length); i++) {
    await grantSouls(ranking[i].wpUserId, REWARD_AMOUNTS[i], `leaderboard:week_${prevWeekKey}:rank${i + 1}`);
  }
}
