...
// Half of the "week you hide = week you sit out" rule — the other half is
// the plain leaderboard_hidden flag, for anyone still hidden right now.
// Shared with state.js, which needs the same answer for the caller's own row
// to tell "sitting out until Monday" apart from "hidden by choice".
...
//
// The week you hide is a week you sit out (locked 2026-08-21, see
// sql/leaderboard-hidden-week.sql): a user also counts as hidden for any
// week their last hide moment (leaderboard_hidden_at) falls inside, even if
// their flag is back off. Checking only the flag's current position let
// someone hide Monday–Saturday, reappear Sunday, and take a prize from a
// week they never competed in public — and cost a Sunday-night hider a
// prize everyone watched them earn all week. Because the window passed in
// here IS the week being asked about, both callers get the right answer
// from the same check: the live ranking keeps a mid-week un-hider off the
// board until Monday, and the Monday grant skips anyone who hid during the
// prize week.
async function computeWeeklyXpRanking(weekStartUtc, weekEndUtc) {
  ...
  const { data: states, error: statesError } = await supabase
    ...
    .select('wp_user_id, streak_count, souls_balance, first_name, avatar_url, leaderboard_hidden, leaderboard_hidden_at')
  ...
  const ranking = wpUserIds.map((wpUserId) => {
    ...
    return {
      ...
      hidden: state?.leaderboard_hidden === true || hidAtSomePointDuring(state, weekStartUtc, weekEndUtc),
    };
  });
...
module.exports = { mostRecentMondayWibUtc, computeWeeklyXpRanking, maybeGrantWeeklyRewards, hidAtSomePointDuring, REWARD_AMOUNTS };
