...
//   'weekly'         — once per local Monday-start week (local_date is
//                       stamped with that week's Monday) — Kisah Awesome Saya:
//                       write as often as you like, XP once a week
//
// cooldownDays (optional, on top of capType) — minimum local-calendar days
// between two grants of the same action, checked against the ledger in
// awardXp. The daily index catches same-day repeats; this catches the days
// in between.
const XP_ACTIONS = {
  ...
  // The 100 XP that used to sit here moved to goal_letter_received (Rheza,
  // 2026-08-24): the reward was always meant for CONQUERING a milestone, not
  // for filling in the creation wizard. Creation keeps a token 10, but only
  // for a NEW CHAPTER — the first milestone ever, or the next one after a
  // conquered one. A mid-goal restart pays nothing. That rule is enforced in
  // awardXp from the ledger itself (see the letter-gate there), not by a
  // time window, so no cooldownDays here.
  milestone_created: { xp: 10, capType: 'daily' },
  // Fires when the user first opens their Surat dari Merlin — the letter is
  // the ceremony of a finished milestone, so in-fiction Merlin hands this XP
  // over with the letter itself. cooldownDays because stages are
  // self-declared taps (deliberately unscored): without a floor, create →
  // confirm 1-2-3 → open letter would be a repeatable 100 XP loop, the
  // biggest faucet in the catalog on a leaderboard that pays Souls. Two
  // weeks per Rheza, 2026-08-24 — a real milestone lives on the scale of
  // months, so honest users never touch this ceiling.
  goal_letter_received: { xp: 100, capType: 'daily', cooldownDays: 14 },
  ...
  // KISAH AWESOME SAYA — the weekly sharing ritual (2026-09-08). Writing pays,
  // once a week; sharing the card never pays anything (no XP, no Souls) — a
  // rewarded share is a manufactured testimonial. The week is the user's own
  // local Monday-start week, same as the leaderboard's.
  kisah_awesome_written: { xp: 25, capType: 'weekly' },
};
...
// The Monday of the local week containing localDate (YYYY-MM-DD). Computed in
// UTC arithmetic on purpose: the string is already the user's local day, so no
// timezone may touch it again.
...
function dedupeDateFor(capType, localDate) {
  ...
  if (capType === 'weekly') return mondayOf(localDate);
...
async function awardXp(wpUserId, actionType, refId, localDate) {
  ...
  // Letter gate for milestone creation: pay only at the start of a new
  // chapter. The ledger tells the story on its own — a milestone_created row
  // only exists when a creation was PAID, so whichever of the two events is
  // most recent decides: no rows = first milestone ever (pay); letter last =
  // the previous milestone was conquered and this is the next one (pay);
  // creation last = the current chapter was already paid for, and re-creating
  // mid-goal earns nothing. Rejections insert no row, so farming attempts
  // never advance this state.
  if (actionType === 'milestone_created') {
    const { data: lastChapterEvent, error: gateError } = await supabase
      .from('xp_events')
      .select('action_type')
      .eq('wp_user_id', wpUserId)
      .in('action_type', ['milestone_created', 'goal_letter_received'])
      .order('created_at', { ascending: false })
      .limit(1);
    if (gateError) throw gateError;
    if (lastChapterEvent && lastChapterEvent.length > 0 && lastChapterEvent[0].action_type === 'milestone_created') {
      return { xpAwarded: 0, alreadyAwarded: true };
    }
  }

  // Time floor between grants, measured in the user's own local days — the
  // daily unique index still backstops same-day repeats, this handles the
  // days in between. Same read-then-write race tolerance as everything else
  // here.
  if (config.cooldownDays) {
    const { data: lastGrant, error: cooldownError } = await supabase
      .from('xp_events')
      .select('local_date')
      .eq('wp_user_id', wpUserId)
    ...
    if (lastGrant && lastGrant.length > 0) {
      const daysSince = Math.round(
        (new Date(localDate).getTime() - new Date(lastGrant[0].local_date).getTime()) / 86400000
      );
...
async function advanceStreak(wpUserId, localDate) {
  ...
  let nextStreak;
  // STREAK RELIGHT (sql/streak-relight.sql): a gap does not throw the old
  // count away any more — it is parked in lapsed_* so the user can relight it
  // for Souls within 3 days. Only written when the cron sweep (lib/
  // streak-relight.js) has not already recorded this same lapse.
  let lapse = {};
  if (!existing || !existing.last_active_date) {
    ...
    if (existing.streak_count >= 1 && existing.lapsed_last_active_date !== existing.last_active_date) {
      lapse = {
        lapsed_streak_count: existing.streak_count,
        lapsed_last_active_date: existing.last_active_date,
