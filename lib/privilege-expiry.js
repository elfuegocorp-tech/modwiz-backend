...
async function pushToUser(wpUserId, { title, body, data }) {
  // Every device of this user, whatever its `enabled` says: `enabled` is the
  // Notifikasi Merlin switch (lib/merlin-nudge.js), and a notice about the
  // member's own access is not Merlin talking (Rheza, 2026-09-13).
  const { data: tokens, error } = await supabase.from('push_tokens').select('token').eq('wp_user_id', wpUserId);
  ...
  for (const row of tokens || []) {
    ...
    if (result.ok) sent += 1;
      ...
      // Uninstalled — deleted, never switched off (see lib/streak-relight.js).
...
// AWESOME BRACELET's free month (sql/bracelet-privilege-trial.sql): the same
// T-7 notice, worded for a month that was a gift with the class rather than
// an alpha test, and pointing at the price that follows it.
const BRACELET_TRIAL_SOURCE = 'bracelet_trial';
const BRACELET_PRICE_LINE = 'Rp49.000 per bulan';

function braceletNoticeBody(date) {
  return `Bulan gratis Modwiz Privilege dari Awesome Bracelet berakhir tanggal ${date}. Lanjutkan ${BRACELET_PRICE_LINE} di app → Toko → Modwiz Privilege.`;
}

function braceletNoticeHtml(expiresAt) {
  const date = formatDateId(expiresAt);
  return [
    `<p>Bulan gratis Modwiz Privilege-mu dari Awesome Bracelet berakhir pada <b>${date}</b>.</p>`,
    ...
    '<li>Nyalakan Kembali streak tidak lagi gratis sekali sebulan.</li>',
    '</ul>',
    `<p>Sebagai pemilik Awesome Bracelet, kamu bisa melanjutkan dengan ${BRACELET_PRICE_LINE}, bukan Rp89.000: buka app → Toko → Modwiz Privilege.</p>`,
    '<p>Rheza &amp; Modwiz</p>',
  ].join('');
}
...
async function runPrivilegeExpirySweep() {
  ...
  // 2. T-7 notice — comps and the bracelet month; a paid subscription
  //    renews on its own.
  const { data: soonRows, error: soonErr } = await supabase
    .from('entitlements')
    .select('id, wp_user_id, expires_at, source')
    .eq('status', 'active')
    .in('source', ['comp', BRACELET_TRIAL_SOURCE])
    .not('expires_at', 'is', null)
  ...
  for (const row of soonRows || []) {
    ...
    const bracelet = row.source === BRACELET_TRIAL_SOURCE;
    await pushToUser(row.wp_user_id, {
      title: 'Modwiz Privilege',
      body: bracelet
        ? braceletNoticeBody(date)
        : `Masa Privilege alpha-mu berakhir tanggal ${date}. Terima kasih sudah menguji bersama kami.`,
      data: { type: 'privilege_expiry', route: '/privilege' },
    });
    const emailed = bracelet
      ? await emailUser(row.wp_user_id, `Bulan gratis Modwiz Privilege-mu berakhir ${date}`, braceletNoticeHtml(row.expires_at))
      : await emailUser(row.wp_user_id, `Masa Modwiz Privilege alpha-mu berakhir ${date}`, noticeHtml(row.expires_at));
    if (emailed) stats.emailed += 1;
