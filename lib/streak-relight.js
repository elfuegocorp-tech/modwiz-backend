...
async function pushToUser(wpUserId, { title, body, data }) {
  ...
  // Notifikasi Merlin switch (lib/merlin-nudge.js), and a streak notice is not
  // Merlin talking — it used to be filtered on it, so muting Merlin silently
  // muted the streak too (Rheza, 2026-09-13). A dead token is deleted below
  // rather than switched off, so `enabled = false` only ever means "Merlin muted".
  ...
  for (const row of tokens || []) {
    ...
    if (result.ok) sent += 1;
      ...
      // Uninstalled. A reinstall registers a new token at login.
