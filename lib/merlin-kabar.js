...
function buildKabarText({ quakeLines = [], trendItems = [], manualLines = [], now = new Date() }) {
  ...
  if (quakeLines.length) {
    lines.push('Gempa (BMKG):', ...quakeLines);
  }
  if (trendItems.length) {
    lines.push(
      'Ramai dicari orang Indonesia 24 jam terakhir (istilah pencarian — satu judul berita yang menyertainya; kamu tidak tahu lebih dari ini):',
