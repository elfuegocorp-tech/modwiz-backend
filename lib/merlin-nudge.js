...
// Register (Rheza, 2026-09-08, applied here 2026-09-13): kamu/aku stay, but
// the vocabulary is baku — tidak/saja/sudah/hanya/untuk, never
// nggak/bikin/aja/udah/cuma/buat — in whole sentences, not poetry. The goal
// feature is TARGET SAYA on screen since 2026-08-25, so a push says "target",
// never "milestone".
const TEMPLATES = {
  // Pagi, lapsed n days: the tappable thing is today's PRIMING.
  pagiLapsed: [
    (n) => `Sudah ${n} hari tanpa check-in. Tidak perlu mengejar hari-hari kemarin. Cukup PRIMING hari ini: tiga goal dalam lima menit.`,
    (n) => `Sudah ${n} hari tanpa PRIMING. Mari mulai lagi pagi ini dengan tiga goal kecil, sisanya bisa menyusul.`,
    (n) => `Aku masih di sini. Sudah ${n} hari ritual pagimu kosong. Isi tiga goal untuk hari ini, itu sudah cukup.`,
  ],
  // Pagi, never checked in at all: invite to the first PRIMING.
  pagiNever: [
    () => `PRIMING pertamamu masih menunggu. Tulis tiga goal untuk hari ini dalam lima menit. Harimu langsung punya arah.`,
    () => `Kita belum pernah memulai pagi bersama. Coba PRIMING hari ini: hanya tiga goal kecil, selesai dalam lima menit.`,
  ],
  // Malam, lapsed n days: the tappable thing is tonight's COSMIC.
  malamLapsed: [
    (n) => `Sudah ${n} hari tidak ada catatan. Tutup malam ini dengan COSMIC. Satu kalimat tentang hari ini pun sudah cukup.`,
    (n) => `Sudah ${n} hari tanpa check-in. Sebelum tidur, isi COSMIC sebentar supaya hari ini tetap tercatat.`,
    (n) => `Sudah ${n} hari aku belum mendengar kabarmu. Malam ini cukup satu refleksi singkat di COSMIC, tidak perlu panjang.`,
  ],
  malamNever: [
    () => `Ritual malammu belum pernah dimulai. Sebelum tidur malam ini, coba COSMIC dan tulis satu kalimat tentang harimu.`,
    () => `Kita belum pernah menutup hari bersama. Coba COSMIC malam ini. Satu kalimat refleksi sudah cukup.`,
  ],
  ...
  deadline: [
    (d) => (d === 0 ? `Deadline targetmu jatuh hari ini. Jika ingin menyusun langkahnya, aku ada di sini.` : `Deadline targetmu tinggal ${d} hari lagi. Jika ingin menyusun langkahnya bersama, aku ada di sini.`),
    (d) => (d === 0 ? `Hari ini adalah deadline targetmu. Satu langkah kecil masih bisa dilakukan. Ceritakan padaku.` : `${d} hari lagi menuju deadline targetmu. Masih ada waktu untuk satu langkah kecil hari ini. Ceritakan padaku.`),
  ],
};
...
/**
 * Users who already got one of the streak's own pushes today (WIB). Reads the
 * stamps lib/streak-relight.js writes on the day it sends each one
 * (lapsed_notified_date, lapsed_last_call_date) — no new column.
 */
async function usersReachedByStreakToday(userIds, today) {
  if (userIds.length === 0) return new Set();
  const { data, error } = await supabase
    .from('gamification_state')
    .select('wp_user_id, lapsed_notified_date, lapsed_last_call_date')
    .in('wp_user_id', userIds);
  if (error) throw error;
  return reachedToday(data || [], today);
}

/** The pure half of usersReachedByStreakToday, kept apart so a test can run it. */
function reachedToday(rows, today) {
  return new Set(rows.filter((r) => r.lapsed_notified_date === today || r.lapsed_last_call_date === today).map((r) => r.wp_user_id));
}
...
async function runDailyNudge(slot) {
  ...
  const stats = { slot, today, devices: 0, lapsed: 0, sent: 0, failed: 0, retired: 0, streakToday: 0 };
  ...
  // ONE "Modwiz reaching out" push per user per day (Rheza, 2026-09-13). The
  // streak's own notices ("padam semalam", "hari terakhir" in
  // lib/streak-relight.js) say the more specific thing, so on a day one of
  // them went out, Merlin's nudge stays quiet. api/merlin-chat.js runs that
  // sweep BEFORE this one, so its stamps are already written when read here.
  const reachedByStreak = await usersReachedByStreakToday(userIds, today);
  ...
  for (const row of candidates) {
    if (reachedByStreak.has(row.wp_user_id)) {
      stats.streakToday += 1;
      continue;
    }
    ...
    if (result.ok) {
      ...
      // The app was uninstalled (or the token is garbage) — delete the row so
      // the cron stops paying for it. A reinstall re-registers at login.
      // Deleted, not switched off: `enabled = false` means only "Merlin
      // muted", which the Streak and Privilege pushes deliberately ignore.
      stats.retired += 1;
...
module.exports = { upsertPushToken, handlePushTokenSync, runDailyNudge, composeNudge, reachedToday };
