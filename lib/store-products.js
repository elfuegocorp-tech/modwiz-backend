...
// PER-USE prices — things paid for EACH TIME, not unlocked once. Kept apart
// from STORE_PRICES on purpose: an id in that table can be debited through
// unlock_product and written to user_unlocks as a permanent door, and a
// per-use product must never be reachable that way (it would open for good at
// one session's price). These are read by the per-use actions in
// api/gamification/spend-souls.js and served to the app on /state as `prices`,
// so the app never carries a copy.
//
// manas_session — one run of Manas Session ("Gunakan Manasmu"). 10 Souls,
// Rheza's decision 2026-09-07, after seeing the token estimate: worst case
// ≈2.400 tokens per session (two Sonnet calls — ~1.700 of system prompt,
// cached after the first run, plus story, facts and two JSON replies). His
// own times-five rule gave 3,29 → 4; he set 10 on top of it, the same way
// Manas (35) sits above the arithmetic of a skill. Two Paket Starter would
// buy four sessions with nothing stranded. If the prompts in
// api/agni-chakti-reading.js grow, revisit.
const CONSUMABLE_PRICES = {
  manas_session: 10,
};

/** What one use of a per-use product costs in Souls, or null for an unknown
 *  id — never quietly free, same rule as priceOf. */
function consumablePriceOf(productId) {
  if (typeof productId !== 'string') return null;
  const souls = CONSUMABLE_PRICES[productId];
