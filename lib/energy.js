...
// Claude.ai's "Current session" + "Weekly limits". The real budget Rheza
// decided is MONTHLY (Free 1.000 Energy, Privilege 3.000); the weekly number
// is that budget divided by 4, so the month is bounded without needing a
// third clock, a new column or a migration. Both are mirrored per-row in
// weekly_energy_max, and getEnergyState's tierChanged check rewrites existing
// rows on their next read — so retuning is a constant edit, nothing else.
// (The earlier 700/2.100 were placeholders that were never decided; they let
// a Free user through ~3.000 Energy a month, 3x the actual budget.)
const WEEKLY_ENERGY_MAX = 250;
const WEEKLY_HOURS = 24 * 7;

// Modwiz Privilege: 3x Freemium on both dials that already exist (Rheza,
// 2026-08-14). Until this existed, the only thing an MP member actually got
// inside the chat was 8 co-work turns instead of 3, which is far too subtle
// for something someone paid for.
const PRIVILEGE_ENERGY_MAX = 300;
const PRIVILEGE_WEEKLY_ENERGY_MAX = 750;
...
async function resolveTier(wpUserId, { fresh = false } = {}) {
  const cached = tierCache.get(wpUserId);
  if (!fresh && cached && Date.now() - cached.at < TIER_TTL_MS) return cached.tier;
...
// `freshTier` skips the 60 s tier cache. merlin-chat asks for it exactly
// once, right before refusing a message for lack of Energy: a person who
// bought Modwiz Privilege from the Energy Habis card seconds ago and is now
// resending the refused message must not be told "habis" again by a cached
// Free tier — the fresh read finds the entitlement and hands the upgrade
// over (the tierChanged path below) in time for that very send.
async function getEnergyState(wpUserId, { freshTier = false } = {}) {
  const [{ data, error }, tier] = await Promise.all([
    supabase.from('energy_state').select('*').eq('wp_user_id', wpUserId).maybeSingle(),
    resolveTier(wpUserId, { fresh: freshTier }),
  ]);
  if (error) throw error;
...
// Buying Extra Energy also turns the pool ON. The switch exists so someone
// can hold Extra Energy back from auto-draining, but a fresh row starts with
// it off, and until 2026-09-23 a first-ever top-up from the Energy Habis
// card bought Energy the next message could not touch — the person paid,
// resent, and was refused again. A purchase is the clearest possible "use
// this"; anyone who wants it held back can flip the switch afterwards.
async function addExtraEnergy(wpUserId, amount) {
  const state = await getEnergyState(wpUserId);
  const nextExtra = state.extraEnergy + amount;
  const next = { ...state, extraEnergy: nextExtra, extraEnergyBarMax: nextExtra, extraEnergyEnabled: true };
  await persist(wpUserId, next);
  return next;
}
