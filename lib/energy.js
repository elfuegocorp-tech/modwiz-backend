...
// Adds `amount` to the caller's current (recharge-applied) Energy, capped at
// their max — the admin top-up / approved-request counterpart to consumeEnergy.
async function grantEnergy(wpUserId, amount) {
  const state = await getEnergyState(wpUserId);
  const next = Math.min(state.energyMax, state.energyCurrent + amount);
...
module.exports = { ENERGY_MAX, RECHARGE_HOURS, tokensToEnergy, getEnergyState, consumeEnergy, grantEnergy };
