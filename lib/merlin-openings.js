...
// Saturday or Sunday of the user's own calendar day (context.today, YYYY-MM-DD).
...
// Which of the three daily rituals is genuinely open for this user at this
// moment — the one fact the block's first rung and the card guard in
// merlin-chat both rest on, kept in one place so the list Merlin reads and
// the card he is allowed to hand over can never disagree.
//
// A ritual is open only inside its own part of the day AND with BOTH halves
// untouched (check-in and meditation are two separate facts); a half-done
// ritual is not a door to point at, it's nagging. PRIMING before 15:00,
// COSMIC from 18:00, IGNITE 11:00–17:00.
//
// Returns null, not an empty set, when the facts are missing — an app build
// that sends no clock or no todayRituals, or a sessions ledger read that
// failed. Null means "no claim either way": the rung stays silent and the
// guard stays permissive, because a data hiccup must never eat a legitimate
// card (2026-09-23: a PRIMING card at 22:21 and a COSMIC card one line after
// the user said he had just done COSMIC — both off-list, both drawn).
function openRituals(context, sessions) {
  const hour = hourOf(context);
  const rituals = context?.todayRituals;
  if (hour === null || !rituals || typeof rituals !== 'object') return null;
  if (!sessions || typeof sessions !== 'object') return null;
  const open = new Set();
  if (hour < 15 && !rituals.morning && sessions.priming === false) open.add('PRIMING');
  if (hour >= 18 && !rituals.evening && sessions.cosmic === false) open.add('COSMIC');
  if (hour >= 11 && hour < 17 && Array.isArray(sessions.ignite) && sessions.ignite.length === 0) open.add('IGNITE');
  return open;
}

const RITUAL_PRIORITY = ['PRIMING', 'COSMIC', 'IGNITE'];
const RITUAL_RUNGS = {
  PRIMING: '- Ritual Pagi PRIMING hari ini belum disentuh sama sekali. → [[CARD:RITUAL:PRIMING]]',
  COSMIC: '- Ritual Malam COSMIC malam ini belum disentuh sama sekali. → [[CARD:RITUAL:COSMIC]]',
  IGNITE: '- Ritual Siang IGNITE hari ini belum dijalani. → [[CARD:RITUAL:IGNITE]]',
};

function buildOpeningsBlock(context, sessions, unlocks, lessonIndex) {
  if (!context || typeof context !== 'object') return { text: '', doors: [] };

  const rungs = [];

  // 1. The ritual of this hour — daily by design, so it outranks everything.
  //    One rung at most, in the order PRIMING → COSMIC → IGNITE; which rituals
  //    count as open is openRituals' call, shared with the card guard.
  const open = openRituals(context, sessions);
  if (open) {
    for (const key of RITUAL_PRIORITY) {
      if (!open.has(key)) continue;
      rungs.push(RITUAL_RUNGS[key]);
      break;
    }
  }

  // 1b. KISAH AWESOME SAYA — the weekly sharing ritual (2026-09-08). Weekend
  //     only, and only while this week's kisah is unwritten. The app sends
  //     context.kisah from its own local record; an older build omits the key
  //     and this rung stays silent rather than nagging about a feature the
  //     phone doesn't have.
  const kisah = context.kisah;
  if (kisah && typeof kisah === 'object' && kisah.thisWeek === false && isWeekend(context.today)) {
    rungs.push(
      '- Akhir pekan, dan Kisah Awesome Saya minggu ini belum ditulis — ritual berbagi mingguan: satu hal yang bergerak di hidupnya, sekecil apa pun, satu kalimat. Yang sederhana buatnya bisa jadi pegangan buat yang belum sampai di posisinya. → [[ACTION:KISAH]]'
    );
  }
    ...
    if (typeof offers.lastDaysAgo === 'number') {
      ...
      rhythm.push(
        offers.lastDaysAgo <= 0
          ? `Terakhir kamu menyodorkan sesuatu${label}: hari ini juga. Jangan mengulang yang itu — tapi praktik LAIN yang masih kosong tetap boleh menyusul di momen wajarnya, satu per balasan (keputusan founder: praktik memang dimainkan berurutan, course tidak).`
  ...
  if (shown.length === 0 && rhythm.length === 0) return { text: '', doors: [] };

  const lines = ['[YANG SEDANG TERBUKA UNTUK DIA]'];
  if (shown.length > 0) {
    lines.push(
      'Fakta yang dihitung dari datanya sendiri, benar SEKARANG. Semua baris di bawah adalah PRAKTIK GRATIS ' +
        'miliknya sendiri yang sedang kosong atau terbengkalai — dan keputusan founder untuk praktik seperti ini: ' +
        'TIDAK perlu terlalu sopan. Kosong = sarankan. Langsung, natural, di tengah percakapan pun boleh — tidak ' +
        'usah menunggu momen sempurna yang tidak pernah datang. Batasnya tetap tiga: maksimal SATU sodoran per ' +
        'balasan, jangan menyodorkan hal yang sama dua kali dalam satu percakapan, dan jangan memotong cerita ' +
        'yang sedang berat atau emosional. Aturan course TIDAK berubah sedikit pun (co-work dulu, cooldown, satu ' +
        'course per siklus) — daftar ini bukan tentang menjual apa pun, tapi tentang kebiasaan yang dia bangun sendiri.'
    );
    lines.push(...shown);
  ...
  // Every rung's marker, extracted for the caller: the handler injects a
  // recency note into the LAST user message naming the fittest one, because
  // three live tests proved a rule this far from the conversation loses to
  // twenty messages of small-talk momentum.
  const doors = shown
    .map((line) => {
      const match = line.match(/\[\[(?:CARD|ACTION):[^\]]+\]\]/);
      return match ? { line, marker: match[0] } : { line, marker: null };
    })
    .filter((door) => door.marker);

  return { text: lines.join('\n'), doors };
}

module.exports = { buildOpeningsBlock, openRituals };
