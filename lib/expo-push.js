...
// will simply find it in the app later", so this logs and returns
// { ok: false } instead of throwing. `error` carries Expo's error code when
// there is one — the daily-nudge cron uses 'DeviceNotRegistered' to stop
// pushing to a token whose app was uninstalled.
...
async function sendExpoPush(token, { title, body, data }) {
  if (!isExpoPushToken(token)) return { ok: false, error: 'InvalidToken' };
  try {
    ...
    if (!res.ok || !ticket || ticket.status === 'error') {
      ...
      return { ok: false, error: (details && details.error) || 'PushFailed' };
    }
    return { ok: true };
  } catch (err) {
